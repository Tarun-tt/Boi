...
function TableComponent() {
  ...
  useEffect(() => {
    axios.post(`${API_URL}/records`, filterParams, { headers }).then((res) => {
  ...
  const handleDelete = (user) => {
    ...
    axios.delete(`${API_URL}/${user.recordId}`, { headers }).then(() => {
      const filteredUser = users.filter((u) => u.recordId !== user.recordId);
  ...
  const handleClear = () => {
    ...
    axios.post(`${API_URL}/records`, resetParams, { headers }).then((res) => {
      setTotalCount(res?.data?.count);
      const mappedUsers = res?.data?.data.map((user) => ({
        ...user,
      }));
  ...
  const handleSearch = () => {
    ...
    axios.post(`${API_URL}/records`, updatedParams, { headers }).then((res) => {
      setTotalCount(res?.data?.count);
      const mappedUsers = res?.data?.data.map((user) => ({
                      ...
                      <td>
                        {hasViewRight && (
                          <button
                            size="sm"
                            onClick={() =>
                              navigate(`/boiform/view/${user.recordId}`)
                            }
                            className="btn"
                          >
                            View
                        ...
                        {hasEditRight && (
                          <button
                            size="sm"
                            onClick={() =>
                              navigate(`/boiform/edit/${user.recordId}`)
                            }
                            className="btn"
                          >
                            Edit
      ...
      <div className="d-flex justify-content-end align-items-center mt-3">
        <nav>
          <ul className="pagination">
            {Array.from({
              length: Math.ceil(totalCount / filterParams.pageSize),
            }).map((_, index) => (
              <li
                key={index}
                className={`page-item ${
                  filterParams.pageNumber === index + 1 ? "active" : ""
                }`}
              >
                <button
                  className="page-link"
